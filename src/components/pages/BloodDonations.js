...
export default class BloodDonations extends Component {
        ...
        <Card key={index+1} fluid color={colorOfCard} style={{marginTop: "30px", marginBottom: "30px"}}>
          <Container style={{padding: '10px'}}>
            ...
            <Table celled>
                <Table.Body>
                    <Table.Row>
                        <Table.Cell width="3" active>Doctor/Nurse Name</Table.Cell>
                        <Table.Cell width="4">{transaction.doctorName}</Table.Cell>
                        <Table.Cell width="3" active>Date</Table.Cell>
  ...
  markAsMedicalError = async (id) => {
    ...
    // mark for reusability 
    await medicalRecordContract.methods.markTransactionAsMedicalError(4, id).send({ from: accounts[0], gas: '200000000' })
    .then(async() => {
      ...
      for(let i = 0; i < cloned.length; i++){
        if (cloned[i].id == id) {
          if (cloned[i].isCorrectionFor !== '') {
            ...
            await medicalRecordContract.methods.markTransactionAsMedicalError(4, cloned[i].isCorrectionFor).send({ from: accounts[0], gas: '200000000' })
          }
      ...
      // mark for reusability 
      toast.success("Blood donation has been set as a medical error.", {
