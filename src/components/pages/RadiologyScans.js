...
export default class RadiologyScans extends Component {
  state = {
    name: '',
    nationalID: '',
    phoneNumber: '',
  ...
  render() {
    ...
    let transactionsCards = transactions.slice(0).reverse().map((transaction, index) => {
      ...
      if (this.isCorrected(transaction.id).result) {
        label = <Label color='red' ribbon>Medical Error : has been corrected by radiology scan with ID ( {this.isCorrected(transaction.id).correctedBy} )</Label> 
        ...
        label = <Label color='blue' ribbon>Is Correction For Radiology Scan With ID ( {transaction.isCorrectionFor} ) </Label> 
      }
        ...
        <Card key={index+1} fluid color={colorOfCard} style={{marginTop: "30px", marginBottom: "30px"}}>
          <Container style={{padding: '10px'}}>
            ...
            <Table celled>
                <Table.Body>
                    <Table.Row>
                        <Table.Cell width="3" active>Radiology Scan Type</Table.Cell>
                        <Table.Cell width="4">{transaction.radiologyType}</Table.Cell>
                        <Table.Cell width="3" active>Radiology Scan Date</Table.Cell>
                        <Table.Cell width="4">{this.formatDate(transaction.date)}</Table.Cell>
                    </Table.Row>
                    <Table.Row>
                        <Table.Cell active>Radiologist Name</Table.Cell>
                        <Table.Cell>{transaction.radiologist}</Table.Cell>
                    ...
                    <Table.Row>
                        ...
                        <Table.Cell>{(transaction.fileHash).substring(0,32)}</Table.Cell>
          ...
          <Card.Description>
            ...
            <Container textAlign="center">
              ...
              <br/>
              <Image
                src={`${imageHasherServer}/storage/files/${transaction.fileHash}`}
                as='a'
                size='medium'
                href={`${imageHasherServer}/storage/files/${transaction.fileHash}`}
                target='_blank'
      ...
      <div>
        <AuthBoilerplate history={this.props.history}>
          <Container padded="true" style={{ padding: "20px" }}>
            <Segment>
              <h3 style={{display: 'inline'}}>List of Radiology Scans</h3> 
              <div style={{position: 'absolute', right: '20px', display: 'inline'}}>
                ...
                <Button 
                  ...
                  labelPosition="left"
                  color="green" 
                  onClick={() => {this.props.history.push(`addRadiologyScan`)}}
                >
                  <Icon name="plus"/>
                  Add new radiology scans
                </Button>
              </div>
            ...
            </Segment>
              {
                ...
                <Segment><Message warning>There are no radiology scans for this patient at the moment</Message></Segment> :
                <div />
              }
            <br/>
            <Card.Group>
  ...
  markAsMedicalError = async (id) => {
    ...
    // mark for reusability 
    await medicalRecordContract.methods.markTransactionAsMedicalError(6, id).send({ from: accounts[0], gas: '200000000' })
    .then(async() => {
      ...
      for(let i = 0; i < cloned.length; i++){
        if (cloned[i].id == id) {
          if (cloned[i].isCorrectionFor !== '') {
            ...
            await medicalRecordContract.methods.markTransactionAsMedicalError(6, cloned[i].isCorrectionFor).send({ from: accounts[0], gas: '200000000' })
          }
      ...
      // mark for reusability 
      toast.success("Radiology scan has been set as a medical error.", {
